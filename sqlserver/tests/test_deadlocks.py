...
def _get_conn_for_user(instance_docker, user):
    # Make DB connection
    conn_str = f"DRIVER={instance_docker['driver']};Server={instance_docker['host']};Database=master;UID={user};PWD={'Password12!'};TrustServerCertificate=yes;"
...
def _create_deadlock(dbm_instance):
    ...
    if "deadlock" in exception_1_text or "deadlock" in exception_2_text:
        return
    raise Exception(f"Couldn't create a deadlock | batch output 1: {exception_1_text} | batch output 2: {exception_2_text}")
...
def test_deadlocks(aggregator, dd_run_check, dbm_instance, convert_xml_to_str):
    ...
    try:
        assert len(deadlock_payloads) == 1, f"Should have collected one deadlock payload, but collected: {len(deadlock_payloads)}"
...
def test_no_empty_deadlocks_payloads(dd_run_check, init_config, dbm_instance, aggregator):
    ...
    with patch.object (
        ...
        assert not _run_check_and_get_deadlock_payloads(dd_run_check, check, aggregator), "shouldn't have sent an empty payload"
