...
@pytest.mark.parametrize(
    'instance_service_tag,init_config_service_tag,expected_service_tag',
    [
        pytest.param('my_service', None, 'service:my_service', id='instance_service_tag'),
        pytest.param(None, 'my_service', 'service:my_service', id='init_config_service_tag'),
        pytest.param(
            'my_service', 'my_service2', 'service:my_service', id='instance_service_tag overrides init_config'
        ),
        pytest.param(None, None, None, id='no_service_tag'),
    ],
)
def test_service_tag(instance, instance_service_tag, init_config_service_tag, expected_service_tag):
    instance['service'] = instance_service_tag
    init_config = {'service': init_config_service_tag}
    config = MongoConfig(instance, init_config, mock.Mock())
    assert config.service_tag == expected_service_tag
